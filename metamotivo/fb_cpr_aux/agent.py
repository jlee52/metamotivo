...
class FBcprAgent(FBAgent):
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            # two heads, main and aux
            next_Qs, next_Qs_aux = self._model._target_critic(next_obs, z, next_action)  # num_parallel x batch x 1
            ...
            # Weights config: [Control Weight, Acceleration Weight]
            weights = [1, 1] 
            dt = 1.0 / 450
            sigma = 1.0

            # Term 1:Action Rate
            action_rate = (next_action - action).pow(2).sum(dim=1)
            action_rate_cost = torch.exp(-action_rate / (2 * 1.0**2))

            # Term 2: Joint Acceleration
            curr_vel = obs[:, 214:]
            next_vel = next_obs[:, 214:]  # You need the previous observation
            acc = (next_vel - curr_vel) / dt
            acc = acc.pow(2).sum(dim=1, keepdim=True)
            acc_cost = torch.exp(-acc / (2 * sigma**2))

            # Final Auxiliary Reward
            aux_reward = weights[0] * action_rate_cost + weights[1] * acc_cost

            aux_Q_mean, aux_Q_unc, next_aux_V = self.get_targets_uncertainty(
                next_Qs_aux,
                self.cfg.train.aux_critic_pessimism_penalty,
            )
            target_Q_aux = aux_reward + discount * next_aux_V
            expanded_aux_targets = target_Q_aux.expand(num_parallel, -1, -1)

        # compute critic loss
        Qs, Qs_aux = self._model._critic(obs, z, action)  # num_parallel x batch x (1 or n_bins)
        
        # compute critic loss
        main_loss = 0.5 * num_parallel * F.mse_loss(Qs, expanded_targets)
        aux_loss = 0.5 * num_parallel * F.mse_loss(Qs_aux, expanded_aux_targets)
        aux_coeff = self.cfg.train.aux_coeff 
        critic_loss = main_loss + aux_coeff * aux_loss
        ...
        with torch.no_grad():
            output_metrics = {
                ...
                "critic_loss_main": main_loss.mean().detach(),
                "mean_disc_reward": reward.mean().detach(),

                # aux head
                "target_Q_aux": target_Q_aux.mean().detach(),
                "Q2": Qs_aux.mean().detach(),
                "mean_next_Q_aux": aux_Q_mean.mean().detach(),
                "unc_Q_aux": aux_Q_unc.mean().detach(),
                "critic_loss_aux": aux_loss.mean().detach(),
                "mean_aux_reward": aux_reward,
    ...
    def update_actor(
        ...
        Qs_discriminator, Qs_aux = self._model._critic(obs, z, action)  # num_parallel x batch x (1 or n_bins)
        ...
        actor_loss = -Q_aux.mean() * self.cfg.train.reg_aux_coeff * weight - Q_discriminator.mean() * self.cfg.train.reg_coeff * weight - Q_fb.mean()
