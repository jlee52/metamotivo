...
class ForwardMap(nn.Module):
    def __init__(self, obs_dim, z_dim, action_dim, hidden_dim, hidden_layers: int = 1, 
        ...
        self.aux = nn.Sequential(*seq)

        # create a NEW head network
        def create_head():
            seq = []
            for _ in range(hidden_layers - 1):
                seq += [linear(hidden_dim, hidden_dim, num_parallel), nn.ReLU()]
        ...
        # Call the function twice to create two INDEPENDENT networks
        self.Fs = create_head()
        self.aux = create_head()
    ...
    def forward(self, obs: torch.Tensor, z: torch.Tensor, action: torch.Tensor):
        ...
        return self.Fs(torch.cat([sa_embedding, z_embedding], dim=-1)), self.aux(torch.cat([sa_embedding, z_embedding], dim=-1))
